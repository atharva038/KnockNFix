  ...
  try {
    ...
    const processedBookings = bookings.map((booking) => {
      ...
      if (!processedBooking.provider) {
        ...
        !processedBooking.provider.user.profileImage ||
        processedBooking.provider.user.profileImage.includes("data:;base64,=")
      ) {
        processedBooking.provider.user.profileImage =
          "https://placehold.co/100x100?text=Provider";
      }
      ...
      if (
        processedBooking.automationStatus &&
        processedBooking.automationStatus.depositProcessed
      ) {
  ...
  try {
    ...
    const processedBookings = bookings.map((booking) => {
      ...
      if (!processedBooking.customer) {
        ...
        !processedBooking.customer.profileImage ||
        processedBooking.customer.profileImage.includes("data:;base64,=")
      ) {
        processedBooking.customer.profileImage =
          "https://placehold.co/100x100?text=Customer";
      }
      ...
      switch (processedBooking.status) {
        case "pending":
          if (processedBooking.automationStatus?.depositProcessed) {
            processedBooking.statusDisplay =
              "Awaiting Provider Confirmation (Deposit Paid)";
          } else {
        ...
        case "confirmed":
          if (processedBooking.automationStatus?.depositProcessed) {
            processedBooking.statusDisplay =
              "Confirmed (Auto-Payment) - Awaiting Work Completion";
          } else {
    ...
    const completedBookings = bookings.filter(
      (b) => b.status === "completed" && b.finalPayment && b.finalPayment.paid
    );
    ...
    completedBookings.forEach((booking) => {
      if (booking.finalPayment && booking.finalPayment.amount) {
        ...
        const commission =
          booking.commission || Math.round(booking.finalPayment.amount * 0.1);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      await Service.findByIdAndUpdate(services, {
    ...
    if (req.body.availability) {
      ...
      days.forEach((day) => {
        ...
        if (
          ...
          !Array.isArray(req.body.availability[day].slots) ||
          req.body.availability[day].slots.length === 0
        ) {
          req.body.availability[day].slots = [
            {
              startTime: "07:00",
        ...
        if (
          req.body.availability[day].isAvailable &&
          req.body.availability[day].slots.length === 0
        ) {
        ...
        req.body.availability[day].slots = req.body.availability[day].slots.map(
          (slot) => ({
...
router.delete("/api/provider/locations/:id", isLoggedIn, async (req, res) => {
  try {
    ...
    // Remove the location
    const locationIndex = provider.serviceAreas.findIndex(
      (area) => area._id.toString() === locationId
    );
